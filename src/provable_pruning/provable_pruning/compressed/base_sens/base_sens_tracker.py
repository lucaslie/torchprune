...
class BaseSensTracker(BaseTracker, ABC):
    ...
    def __init__(self, module, absolute=False):
        ...
        # define proper unfold function for sensitivity computations
        if isinstance(self.module, nn.Conv2d):
            ...
            self.unfold = lambda x: x.unsqueeze(-1)
    ...
    def _store_stats(self, z_plus, z_minus, bias):
        """Store some stats for later use, e.g., shrinkage."""
        pass
    ...
    def _update_g_sens(self, activations, z_no_bias, outs):
        ...
        self._store_stats(z_plus, z_minus, self.module.bias)
    ...
    def _reshape_g_sens(self, g_sens, batch_size):
        g_sens = g_sens.unsqueeze(0)
        g_sens = g_sens.float() * (
            torch.ones(batch_size, g_sens.shape[1]).float().to(g_sens.device)
        )
        g_sens = g_sens.unsqueeze(-1)
        return g_sens
