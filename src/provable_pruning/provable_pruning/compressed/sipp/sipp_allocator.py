...
class SiPPRandAllocator(BaseAllocator):
    ...
    def _allocate_method(self, keep_ratio, size_original):
        # total sample budget
        m_total = int(keep_ratio * float(size_original))
        ...
        constraints = [
            ...
            self._budget_constraint(x_arg, m_total),
        ]
...
class SiPPAllocator(BaseAllocator):
    ...
    def _get_top_indicator(self, arr, num_samples):
        ...
        if torch.sum(torch.as_tensor(num_samples)) > 1:
            # compute top indicator
            idx_top = np.argpartition(arr.cpu().numpy(), -int(num_samples))[
                -int(num_samples) :
            ]
            indicator[idx_top] = True

        # return in original shape
        return indicator.view(shape)
    ...
    def _allocate_method(self, keep_ratio, size_original):
        ...
        # get budget
        budget = self._get_budget(keep_ratio, size_original)
    ...
    def _get_budget(self, keep_ratio, size_original):
        return int(keep_ratio * float(size_original))
...
class SiPPHybridAllocator(BaseAllocator):
    ...
    def _allocate_method(self, keep_ratio, size_original):
        self._allocator_rand._allocate_method(keep_ratio, size_original)
